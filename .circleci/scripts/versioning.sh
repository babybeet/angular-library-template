...
function bumpPackageVersion {
    if [ $(matchCommitMessagePattern "build|chore|ci|docs|fix|perf|refactor|style|test") != "0" ]; then
        ...
        echo "Commit message should start with a valid type, please see https://github.com/babybeet/angular-library-template/blob/main/CONTRIBUTING.md to learn more"
        npm version minor --no-git-tag-version
    fi
...
if [ $(matchCommitMessagePattern "feat|fix|build|chore|ci|docs|perf|refactor|style|test|breaking(\s*change)?") != "0" ]; then
    ...
    cd projects/angular-library-template
    ...
    git add {.,projects/angular-library-template}/package.json
